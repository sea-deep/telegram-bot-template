...
import { ChosenInlineResult } from "../structures/ChosenInlineResult.js";
import { resolveFiles } from "./pathResolver.js";
import { Logger } from "../helpers/Logger.js";

export const chosenResultsMap = new Map<string | RegExp | Array<string | RegExp>, ChosenInlineResult>();

export async function loadChosenResults(bot: Telegraf<Context>): Promise<void> {
  ...
  try {
    ...
    for (const fileUrl of fileUrls) {
      ...
      const result: ChosenInlineResult = module.default;
...
async function executeResult(ctx: Context, bot: Telegraf<Context>, result: ChosenInlineResult) {
  try {
    await result.execute(ctx as unknown as import("../structures/ChosenInlineResult.js").ChosenContext, bot);
  } catch (error) {
